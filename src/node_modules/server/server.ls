...
class Server extends EventEmitter
	...
	/**
	 * Finds a port to run the server system, asks the latter to run on it and configures it.
	 */
	run: ->
		onPortFound = !(port) ~>
			@ <<< {port}
			...
			let self = @ =>
				...
				for name in startEventsNames
					self.emit name, self, port

		options = @options.network with {cb: onPortFound}
		network.execOnAvailablePort options
