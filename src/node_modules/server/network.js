...
var PortsSpec = oop.Class({
	...
	schema: {
		properties: [
			{names: ['preferred', 'prefered'], type: oop.types.Number, 'default': 3000},
			{name: 'min', type: oop.types.Number, 'default': 49152},
...
var findAvailablePort = oop.methodFactory({
	...
	exec: function(spec) {
		portscanner.checkPortStatus(spec.ports.preferred, spec.hostname, function(error, status) {
			if (status === 'closed') {
				spec.cb(spec.ports.preferred);
			} else {
